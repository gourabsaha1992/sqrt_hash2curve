...
int main(void) {   
	...
	printf("Using curve: bls24-317, SWU\n");
   	MEASURE(ep_map_sswum(P, (const uint8_t *)msg, msg_len);)
	...
	printf("Using curve: bls24-317, SwiftEC\n");
	MEASURE(ep_map_swift(P, (const uint8_t *)msg, msg_len);)
	printf("RDTSC_clk_min=%f\n",RDTSC_clk_min);
	printf("RDTSC_clk_median=%f\n",RDTSC_clk_median);
	printf("RDTSC_clk_max=%f\n",RDTSC_clk_max);
