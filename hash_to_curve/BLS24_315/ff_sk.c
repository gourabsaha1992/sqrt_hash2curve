	...
	RLC_TRY {
		...
		fp_prime_conv(t2, kk);
		    ...
		    ep_set_infty(p);
	            ...
	            fp_sqr(tmp1, t1);          // t1^2
		    //fp_print(tmp1);
		    fp_mul_dig(tmp1,tmp1,13);   // c*t1^2
		    //fp_print(tmp1);
		    fp_inv(tmp2,tmp1);     // 1/ct1^2
		    fp_add_dig(x,tmp2,1);
		    fp_mul(x2,x,tmp1);
		    //fp_print(x);
		    //fp_print(x2);
...
void ep_map_ff(ep_t p, const uint8_t *msg, size_t len,  fp_t b, fp_t c, fp_t D, fp_t A, fp_t B, fp_t alpha) {
	...
	RLC_TRY {
		static const uint8_t DST[] = "FF-BLS24-315-G1";
...
int main(void) {   
    ...
    // Get curve parameter b and lambda
        fp_set_dig(b, 1);
        fp_set_dig(c, 13);
        bn_read_str(tmp,"4C23A02B586D650D3F7498BE97C5EAFDEC1D01AA27A1AE0421EE5DA52BDE5026FE802FF40300000",112,16);
	...
	const char *msg = "Hashing to BLS24-315 and this is the msg";
