...
void koshelov_a0_implt(ep_t p, const uint8_t *random, size_t len, fp_t c[8], fp_t w[8],bn_t e,fp_t B,fp_t bs) {
	...
	RLC_TRY {
		...
		fp_mul_dig(num0,s12,81);//num0=2^4*s12
		...
		fp_mul_dig(temp1,s22,6561);
		fp_add(num1,num1,temp1);
		fp_add(den,den,temp1);
		
		fp_mul_dig(temp1,s2,81);
		fp_dbl(temp1,temp1);
		fp_neg(temp2,temp1);
...
void ep_map_koshelov(ep_t p, const uint8_t *msg, size_t len, fp_t c[8], fp_t w[8],bn_t e,fp_t B,fp_t bs) {
	...
	RLC_TRY {
		static const uint8_t DST[] = "KOSHELOV-BLS24-315-G1";
...
int main(void) {   
	...
	bn_new(tmp);
	bn_read_str(tmp,
    "130dab75e66410d74163012df52170eb98f15488f36333295b4150f8208fee00bfcffffe",
	...
	bn_read_str(tmp,
    "4c1f6675ff0ea829f5b8ebe900246ab709383d6a5e5904e12f5c09b371093e5fa9e6aae3eea29be",
    115,   // number of hex chars
    16);
	...
	bn_read_str(tmp,
    "298b432951d79a62a4dc1176e843674d5e993f7bc8be89e2528f8748f48ee037c612ba47ceedeae",
    115,   // number of hex chars
    16);
	...
	bn_read_str(tmp,
    "525fd7ae767d4839f906dfe60a6ef55323319eae7f1ba3c777a81fc7d67bd4f24c2fa33d547c64",
    115,   // number of hex chars
    16);
...
fp_t b,b2,b3,b4,bs,B;
	...
	bn_add_dig(e, e, 7);
	bn_div_dig(e, e, 27);
    const char *msg = "hashing to BLS24-315 and this is the message";
